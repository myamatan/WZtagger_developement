...
void effBkg_comp(){
	...
	for(UInt_t i=0; i<jetsize; i++){
		...
		TString fname = "../input/merged_"+jet.at(i).at(0)+"_new/allMC/allMC." + ch + ".root";
	...
	for(UInt_t i=0; i<jetsize; i++){
			setEfficiency(hbef[i], haft[i], heff[i],"rejection");
	}
	...
	leg= new TLegend(0.4,0.60,0.9,0.8);
        leg->SetBorderSize(0);
        lat = new TLatex(250, 100, "#it{ATLAS} #bf{Simulation Work in progress}");
        latsub = new TLatex(400, 90, "#bf{"+boson+" tagger}");
	cv->cd();
	for(UInt_t i=0; i<jetsize; i++){
		if( i==0 ) heff[i]->Draw();
		else heff[i]->Draw("same");
		heff[i]->SetStats(0);
		heff[i]->GetYaxis()->SetRangeUser(0,110);
		heff[i]->GetYaxis()->SetTitle("Background rejection (1/eff)");
	...
	lat->Draw();
	latsub->Draw();
	cv->Print("../output/bkgRejection_comp." + ch + "_" + boson + "_" + nameOption + ".eps");
