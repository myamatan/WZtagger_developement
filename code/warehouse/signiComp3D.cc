...
float cutProvider(float pt){
	float cut = pt > 600 ? 0.5 : 0.7;
	return cut;
}
...
void signiComp3D(){
	...
	for(auto i = combs.begin(); i != combs.end(); ++i){
		...
		std::cout << "D2 cut : " << cutWPs.at(2) << std::endl;
		cutWPs.clear();
		// Upper M set
		wpfile = std::string("../output/WPs/SmoothedContained"+tagger+wps[i->second.at(1)]);
		cutWPs = retrieveWPs( wpfile );
		fMU[n] = new TF1(Form("MU_%i",n), cutWPs.at(1), 200, 2500);
		std::cout << "MU cut : " << cutWPs.at(1) << std::endl;
		cutWPs.clear();
		// Lower M set
		wpfile = std::string("../output/WPs/SmoothedContained"+tagger+wps[i->second.at(2)]);
		cutWPs = retrieveWPs( wpfile );
		fML[n] = new TF1(Form("ML_%i",n), cutWPs.at(0), 200, 2500);
		std::cout << "ML cut : " << cutWPs.at(0) << std::endl;
		cutWPs.clear();
		// KtDR set
		wpfile = std::string("../output/WPs/SmoothedContained"+tagger+wps[i->second.at(0)]);
		cutWPs = retrieveWPs( wpfile );
		//if( cutWPs.at(3)!="" ) is3D.push_back(true);
		//else is3D.push_back(false);
		if( TString(i->second.at(0)).Index("3D")!=-1 ) is3D.push_back(true);
		else is3D.push_back(false);
		std::cout << "KtDR cut : " << cutWPs.at(3) << std::endl;
		fKtDR[n] = new TF1(Form("KtDR_%i",n), cutWPs.at(3), 200, 2500);
		cutWPs.clear();
		// Hist set
	...
	for(long l=0; l<nEventB; l++){
		...
		for(int i=0; i<Ncombs; i++){
			...
			mcutu = fMU[i]->Eval(pt);
			mcutl = fML[i]->Eval(pt);
			ktdrcut = !is3D.at(i) ? 999 : fKtDR[i]->Eval(pt);
			//ktdrcut = !is3D.at(i) ? 99999. : cutProvider(pt);
			if( d2 < d2cut &&
    			    m < mcutu &&
			    m > mcutl &&
			    ktdr < ktdrcut &&
